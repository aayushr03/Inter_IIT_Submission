...
c.execute(''' CREATE TABLE ldapgrade(id TEXT, no TEXT, name TEXT,credits TEXT, grades TEXT)''')
#____________
@app.route('/')
def index():
...
def getValue():
    ...
    for j in range(0,22):
        c.execute('''INSERT INTO ldapgrade VALUES (?,?,?,?,?)''',(courseid[j],courseno[j],coursename[j],coursecredits[j],coursegrades[j]))
        conn.commit()
    c.execute('''SELECT * FROM ldapgrade''')
    res=c.fetchall()
    print(res)
...
@app.route('/user_info')
def users():
    userdetails=c.execute('''SELECT * FROM ldapgrade''')
